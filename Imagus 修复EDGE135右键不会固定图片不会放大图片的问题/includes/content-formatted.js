...
for(;
i<hosts.length;
++i)if(sf.host===i||sf.host===void 0&&hosts[i][0][0]==="+")urls.push(hosts[i][1].replace("%url",encodeURIComponent(sf.url)).replace("%raw_url",
sf.url));
Port.send({
cmd:"open",url:urls,nf:!!sf.nf
}
)
}
;
var shortcut={
specKeys:{
8:"Backspace",9:"Tab",13:"Enter",16:"shift",17:"ctrl",18:"alt",27:"Esc",32:"Space",33:"PgUp",34:"PgDn",35:"End",36:"Home",37:"Left",38:"Up",39:"Right",40:"Down",45:"Ins",46:"Del",96:"0",97:"1",98:"2",99:"3",100:"4",101:"5",102:"6",103:"7",104:"8",105:"9",106:"*",107:"+",109:"-",110:".",111:"/",173:"-",186:";
",187:"=",188:",",189:"-",190:".",191:"/",192:"`",219:"[",220:"\\",221:"]",222:"'",112:"F1",113:"F2",114:"F3",115:"F4",
...
if(imgs)if(Array.isArray(imgs=imgs.match(/\burl\(([^'"\)][^\)]*|"[^"\\]+(?:\\.[^"\\]*)*|'[^'\\]+(?:\\.[^'\\]*)*)(?=['"]?\))/g))){
var i=imgs.length;
...
if(e.type==="mouseup"){
if(e.target!==
PVI.CNT||PVI.fullZm||e.button!==0)return;
...
if(e.clientX>=d.clientWidth||e.clientY>=d.clientHeight)return;
...
if(platform.opera&&(e.target.clientWidth&&e.offsetX>=e.target.clientWidth||e.target.clientHeight&&e.offsetY>=e.target.clientHeight))return;
if(e.target===PVI.CNT){
PVI.md_x=e.clientX;
PVI.md_y=e.clientY;
return
...
"relatedTarget":PVI.TRG,"clientX":e.clientX,"clientY":e.clientY
}
);
...
console.log("[Imagus Debug] onMouseDown right button - actTrigger:",cfg.hz.actTrigger,"fireHide:",PVI.fireHide,"target:",e.target);
...
PVI.md_x=e.clientX;
PVI.md_y=e.clientY;
console.log("[Imagus Debug] onMouseDown set mdownstart:",mdownstart,"md_x:",PVI.md_x,"md_y:",PVI.md_y);
if(e.target===PVI.CNT||e.target===PVI.IMG||e.target===PVI.VID){
e.preventDefault();
e.stopPropagation()
}
else if(platform.chrome&&(e.target.href||(d=e.target.parentNode)&&d.href))e.preventDefault()
}
;
var onContextMenu=function(e){
console.log("[Imagus Debug] onContextMenu called - button:",e.button,"mdownstart:",mdownstart,"PVI.md_x:",PVI.md_x,"e.clientX:",e.clientX);
if(!mdownstart||e.button!==
2||PVI.md_x!==e.clientX||PVI.md_y!==e.clientY){
console.log("[Imagus Debug] Early return - mdownstart:",mdownstart,"button:",e.button,"moved:",PVI.md_x!==e.clientX||PVI.md_y!==e.clientY);
...
console.log("[Imagus Debug] onContextMenu - mdownstart:",mdownstart,"elapsed:",e.timeStamp-mdownstart,"state:",PVI.state,"target:",e.target,"fzOnPress:",cfg.hz.fzOnPress);
var i;
var elapsed=e.timeStamp-mdownstart>=300;
mdownstart=null;
i=PVI.state>2&&(elapsed&&cfg.hz.fzOnPress===2||!elapsed&&!PVI.fullZm&&cfg.hz.fzOnPress===1);
console.log("[Imagus Debug] i=",i,"elapsed=",elapsed,"fullZm=",PVI.fullZm);
if(i)PVI.key_action({
which:13,shiftKey:PVI.fullZm?true:e.shiftKey
}
);
else if(i=PVI.state<3&&PVI.SRC&&
PVI.SRC.m2!==void 0){
if(elapsed)return;
PVI.load(PVI.SRC.m2);
PVI.SRC=void 0
...
console.log("[Imagus Debug] After checks - i=",i,"target===PVI.CNT:",e.target===PVI.CNT);
if(i){
pdsp(e);
e.preventDefault();
e.stopPropagation();
...
else if(e.target===PVI.CNT||e.target===PVI.IMG||e.target===PVI.VID){
pdsp(e,false);
e.preventDefault();
e.stopPropagation();
e.stopImmediatePropagation();
...
else if(e.ctrlKey&&!elapsed&&!e.shiftKey&&!e.altKey&&cfg.tls.opzoom&&PVI.state<2&&(i=checkIMG(e.target)||checkBG(win.getComputedStyle(e.target).backgroundImage))){
...
var PVI={
TRG:null,
DIV:null,IMG:null,CAP:null,HLP:doc.createElement("a"),anim:{
...
,resolving:[],lastTRGStyle:{
cursor:null,outline:null
}
,iFrame:false,state:null,rgxHTTPs:/^https?:\/\/(?:www\.)?/,pageProtocol:win.location.protocol.replace(/^(?!https?:).+/,"http:"),palette:{
"load":"rgb(255, 255, 255)","R_load":"rgb(255, 204, 204)","res":"rgb(222, 255, 205)","R_res":"rgb(255, 234, 128)","R_js":"rgb(200, 200, 200)","pile_fg":"#000","pile_bg":"rgb(255, 255, 0)"
}
...
PVI.CNT=PVI.IMG;
PVI.DIV.IMGS_=PVI.DIV.IMGS_c=PVI.LDR.IMGS_=
PVI.LDR.IMGS_c=PVI.VID.IMGS_=PVI.VID.IMGS_c=PVI.IMG.IMGS_=PVI.IMG.IMGS_c=true;
PVI.DIV.style.cssText="margin: 0;
 padding: 0;
 "+(cfg.hz.css||"")+";
 visibility: visible;
 cursor: default;
...
PVI.LDR.style.cssText=(cfg.hz.LDRcss||"padding: 5px;
 ...
 "+
(cfg.hz.LDRanimate?platform["transition_css"]+": background-color .5s, opacity .2s ease, top .15s ease-out, left .15s ease-out":"");
PVI.LDR.src=cfg.hz.LDRsrc||"data:image/svg+xml;
base64,";
...
if(platform.chrome||platform.maxthon)PVI.VID.poster="data:image/gif;
...
else if(cfg.hz.hideIdleCursor>=0)PVI.IMG.style.cursor=platform.opera?"text":"none";
PVI.DIV.addEventListener("dragstart",function(e){
pdsp(e,false)
}
,true);
...
y={
"mt":"marginTop","mr":"marginRight","mb":"marginBottom","ml":"marginLeft","bt":"borderTopWidth","br":"borderRightWidth","bb":"borderBottomWidth","bl":"borderLeftWidth","pt":"paddingTop","pr":"paddingRight","pb":"paddingBottom",
"pl":"paddingLeft"
}
;
for(z in y){
...
PVI.anim={
maxDelay:0,opacityTransition:function(){
...
if(x[y+"Property"]){
p=/,\s*/;
p=[x[y+"Property"].split(p),x[y+"Duration"].replace(/initial/g,
"0s").split(p)];
PVI.anim.css=x[y]||PVI.DIV.style[y];
["opacity","left","top","width","height"].forEach(function(el){
var idx=p[0].indexOf(el),val=parseFloat(p[1][idx])*1E3;
if(val>0&&idx>-1){
PVI.anim[el]=val;
...
PVI.DIV.firstChild.style.cssText="z-index: -1;
 width: 100%;
 height: 100%;
 position: absolute;
 left: 0;
...
tag:"b",attrs:{
style:"display: none;
 "+platform["transition_css"]+": background-color .1s;
 border-radius: 3px;
 padding: 0 2px"
}
...
PVI.CAP.style.cssText="left:0;
 ...
 background:rgba('+
(n?"255,255,255,.95":"0,0,0,.75")+") !important;
 color:#"+(n?"000":"fff")+" !important;
 box-shadow: 0 0 1px #"+(n?"666":"ddd")+" inset;
 padding:0 4px;
 border-radius: 3px";
...
PVI.HLP.innerHTML=caption.replace(/<[^>]+>/g,
"").replace(/</g,"&lt;
");
...
if(PVI.TRG.IMGS_album)if(c.firstChild.style.display==="none"&&(h=PVI.stack[PVI.TRG.IMGS_album])&&h[2]){
...
,attrObserver:function(target,
isStyle,oldValue){
if(isStyle){
var bgImage=target.style.backgroundImage;
if((!bgImage||oldValue.indexOf(bgImage.slice(5,-2))!==-1)&&oldValue&&oldValue.indexOf("opacity")===-1&&target.style.cssText.indexOf("opacity")===-1)return
}
...
var target=e.target;
switch(e.attrName){
case "style":var bgImg=target.style.backgroundImage;
if((!bgImg||e.prevValue.indexOf(bgImg.slice(5,-2))!==-1)&&e.prevValue.indexOf("opacity")===-1&&target.style.cssText.indexOf("opacity")===
-1)return;
case "href":case "src":case "title":case "alt":if(target===PVI.TRG)PVI.nodeToReset=target;
...
else node.addEventListener("DOMAttrModified",PVI.onAttrChange)
}
...
if(isHTMLElement)if(el.childElementCount>
0&&el.childElementCount<3){
imgs=el.firstElementChild;
if(imgs.childElementCount&&imgs.childElementCount<4)if(imgs.firstElementChild.localName==="img")imgs=imgs.firstElementChild;
...
else if(!el.childElementCount&&el.parentNode.childElementCount<=5&&(el.localName==="img"?el.src.lastIndexOf("data:",0)===0||el.naturalWidth<
3||el.naturalHeight<3||el.style.opacity==="0":!/\S/.test(el.textContent))&&el.style.backgroundImage[0]!=="u"){
p=el.previousElementSibling;
[p&&p.previousElementSibling,p,el.nextElementSibling].some(function(sib){
if(sib&&sib.localName==="img"&&sib.offsetParent===el.offsetParent&&Math.abs(sib.offsetLeft-el.offsetLeft)<=10&&Math.abs(sib.offsetTop-el.offsetTop)<=10&&Math.abs(sib.clientWidth-el.clientWidth)<=30&&Math.abs(sib.clientHeight-el.clientHeight)<=30){
el=sib;
return true
}
...
if(el.clientWidth>topWinW*
.7&&el.clientHeight>topWinH*.7)return null;
imgs={
imgSRC_o:el.currentSrc||el.src||el.data||null
}
;
...
if(imgs.imgSRC_o)imgs.imgSRC=
imgs.imgSRC_o.replace(PVI.rgxHTTPs,"")
}
if(!isHTMLElement)return imgs.imgSRC?imgs:null;
if(el.style.backgroundImage[0]==="u")imgs.imgBG_o=el.style.backgroundImage;
else if(el.parentNode){
p=el.parentNode;
if(p.offsetParent===el.offsetParent&&p.style&&p.style.backgroundImage[0]==="u")if(Math.abs(p.offsetLeft-el.offsetLeft)<=10&&Math.abs(p.offsetTop-el.offsetTop)<=10&&Math.abs(p.clientWidth-el.clientWidth)<=30&&Math.abs(p.clientHeight-el.clientHeight)<=30)imgs.imgBG_o=p.style.backgroundImage
}
if(!imgs.imgBG_o)return imgs.imgSRC?
imgs:null;
...
Function("var $ = arguments;
 "+(inline?"return ":"")+rule[param].slice(1)).bind(PVI)
}
catch(ex){
...
if(url[1]==="/")if(url[0]==="/")url=PVI.pageProtocol+url;
else if(url[0]==="#"&&url[2]==="/")url="#"+PVI.pageProtocol+url.slice(1);
return url
}
,normalizeURL:function(url){
if(url[1]==="/"&&url[0]==="/")url=PVI.pageProtocol+
url;
PVI.HLP.href=url;
return PVI.HLP.href
}
...
if(cfg.sieve[rule.id].res===
1)rule.req_res=true;
else if(rule.skip_resolve)if(typeof cfg.sieve[rule.id].res==="function"){
params.url=[URL];
return PVI.onMessage({
"cmd":"resolved","id":-1,"m":false,"return_url":true,"params":params
}
)
}
...
null;
Port.send({
"cmd":"resolve","url":URL,"params":params,"id":PVI.resolving.push(trg)-1
}
)
}
...
var i=0,n=trg,ret=false,URL,rule,imgs,use_img,tmp_el,attrModNode;
do{
if(n.nodeType!==void 0)if(n.nodeType!==1||n===doc.body)break;
...
if(trgRect.left-10<=elRect.left&&trgRect.right+10>=elRect.right&&trgRect.top-
10<=elRect.top&&trgRect.bottom+10>=elRect.bottom)imgs=PVI.getImages(tmp_el[i],true)
}
break
}
...
ret=rule.url?URL.replace(rule[tmp_el?"img":
"link"],rule.url):URL;
ret=PVI.resolve(PVI.httpPrepend(ret||URL,n.href.slice(0,n.href.length-URL.length)),{
id:i,$:[n.href].concat((URL.match(rule[tmp_el?"img":"link"])||[]).slice(1)),loop_param:tmp_el?"img":"link",skip_resolve:ret===""
}
,trg.IMGS_TRG||trg)
}
...
for(;
rule=cfg.sieve[i];
++i)if(use_img[0]||rule.img&&(imgs.imgSRC&&rule.img.test(imgs.imgSRC)||imgs.imgBG&&(use_img[1]=rule.img.test(imgs.imgBG)))){
...
ret=PVI.resolve(PVI.httpPrepend(ret,imgs.slice(0,imgs.length-URL.length)),{
...
if(trg.nodeType===1){
attrModNode=trg;
if(cfg.hz.history)trg.IMGS_nohistory=
true
}
break
}
...
rule=ret;
ret=PVI.find({
href:ret,IMGS_TRG:trg.IMGS_TRG||trg,IMGS_loop_count:1+(trg.IMGS_loop_count||0)
}
);
...
URL=/^https?:\/\//;
URL=[n&&n.href&&n.href.replace(URL,""),
...
naturalHeight:trg.naturalHeight,src:null
}
;
for(i=0;
i<tmp_el.length;
...
if(rule.src)rule.naturalHeight*=
rule.naturalWidth/trg.naturalWidth;
...
if(trg.IMGS_fallback_zoom!==rule)trg.IMGS_fallback_zoom=trg.IMGS_fallback_zoom?
[trg.IMGS_fallback_zoom,rule]:rule
}
else if(ret){
...
n=trg;
do{
if(n.IMGS_caption||n.title&&(!trg.hasAttribute("src")||trg.src!==n.title))trg.IMGS_caption=
n.IMGS_caption||n.title;
if(i===0&&!cfg.hz.capNoSBar)trg.title="";
if(trg.IMGS_caption)break
}
while(++i<=5&&(n=n.parentNode)&&n.nodeType===1);
if(!trg.IMGS_caption)if(trg.alt&&trg.alt!==trg.src&&trg.alt!==imgs)trg.IMGS_caption=trg.alt;
else if(tmp_el&&cfg.hz.capLinkText)trg.IMGS_caption=tmp_el;
if(trg.IMGS_caption)if(!cfg.hz.capLinkText&&trg.IMGS_caption===tmp_el||trg.IMGS_caption===trg.href)delete trg.IMGS_caption;
...
vdfDpshPtdhhd:"from_frame",msg:msg
}
,"*");
return
}
...
(PVI.CNT===
PVI.IMG?PVI.VID:PVI.IMG).style.display="none";
...
if(box.opacity==="0"&&(PVI.BOX===PVI.DIV&&PVI.anim.opacity||PVI.BOX===PVI.LDR&&cfg.hz.LDRanimate))if(PVI.state===2)PVI.anim.opacityTransition();
...
box=PVI.DBOX;
ratio=w/h;
var fs=cfg.hz.fullspace||cfg.hz.placement===2,cap_size=PVI.CAP&&PVI.CAP.overhead&&!(PVI.DIV.curdeg%360)&&PVI.CAP.state!==0&&(PVI.CAP.state===2||
PVI.TRG.IMGS_caption&&cfg.hz.capText||PVI.TRG.IMGS_album||cfg.hz.capWH)?PVI.CAP.overhead:0,vH=box["wm"]+(rot?box["hpb"]:box["wpb"]),hH=box["hm"]+(rot?box["wpb"]:box["hpb"])+cap_size,vW=Math.min(w,(fs?winW:x<rSide?rSide:x)-vH),hW=Math.min(w,winW-vH);
...
case 1:hH=(x<rSide?rSide:x)<vW;
if(hH&&cfg.hz.fullspace&&(winH-vH<=winW-vW||vW<=(x<rSide?rSide:x)))hH=false;
left=x-(hH?vW/2:x<rSide?0:vW);
top=y-(hH?y<bSide?0:vH:vH/2);
break;
case 2:left=(winW-vW)/2;
top=(winH-vH)/2;
hW=false;
break;
case 3:left=x<rSide||vW>=PVI.x&&winW-PVI.x>=vW?PVI.TBOX.right:x-vW;
top=y<bSide||vH>=PVI.y&&winH-PVI.y>=vH?PVI.TBOX.bottom:y-vH;
hH=
(x<rSide?rSide:x)<vW||(y<bSide?bSide:y)>=vH&&winW>=vW&&(PVI.TBOX.width>=winW/2||Math.abs(PVI.x-left)>=winW/3.5);
...
if(hW)if(hH||(x<rSide?rSide:x)<vW||winH<vH){
...
switch(typeof idx){
case "boolean":idx=idx?1:album.length-1;
break;
case "number":idx=album[0]+(idx||0);
break;
default:if(/^[+-]?\d+$/.test(idx)){
i=parseInt(idx,10);
...
s=album[0];
i=s+1;
for(i=i<album.length?i:1;
i!==s;
++i<album.length?0:i=1)if(album[i][1]&&idx.test(album[i][1])){
idx=i;
break
}
...
i=PVI.TRG;
win.parent.postMessage({
vdfDpshPtdhhd:"from_frame","src":src,"thumb":i.IMGS_thumb?[i.IMGS_thumb,i.IMGS_thumb_ok]:null,"album":i.IMGS_album?{
"id":i.IMGS_album,"list":PVI.stack[i.IMGS_album]
}
:null,"caption":i.IMGS_caption
}
,"*");
return
}
...
if(cfg.hz.thumbAsBG){
if(PVI.interlacer)PVI.interlacer.style.display=
"none";
...
PVI.timers.onReady=setInterval(PVI.content_onready,
...
if(autoplay&&platform.opera)setTimeout(function(){
...
while(src_left===src);
if(!src_res_arr||!src_res_arr.length)if(src_left)t.IMGS_c_resolved=src_left;
...
var url,i,n;
if(!PVI.CNT||!PVI.TRG||platform.crx&&
chrome.extension.inIncognitoContext||platform.maxthon)return;
if(manual){
...
!platform.safari){
Port.send({
cmd:"history",url:url,manual:manual
}
);
return
}
...
if(!PVI.TRG||
!reset&&(cfg.hz.capWH||PVI.TRG.IMGS_HD===void 0))return;
if(reset){
...
if(PVI.CNT===PVI.IMG&&
!PVI.IMG.loaded&&cfg.hz.thumbAsBG&&PVI.TRG.IMGS_thumb!==false&&!PVI.TRG.IMGS_album){
var inner_thumb,w,h;
...
w=
(inner_thumb||PVI.TRG).clientWidth;
...
h.appendChild(doc.createElement("div")).style.cssText="width: 100%;
 height: 100%;
 background-color: rgba("+
(w>>16)+","+(w>>8&255)+","+(w&255)+","+parseFloat(cfg.hz.thumbAsBGOpacity)+")"
}
w=h.style;
w.cssText="position: absolute;
 top: 0;
 left: 0;
 width: 100%;
 height: 100%;
...
if(platform.opera&&PVI.TRG.IMGS_SVG)PVI.DIV.replaceChild(PVI.IMG,PVI.IMG);
...
if(PVI.iFrame){
win.parent.postMessage({
vdfDpshPtdhhd:"from_frame","hide":true
}
,"*");
return
}
...
if(animDIV&&
PVI.anim.top||animLDR)PVI.BOX.style.top=(cfg.hz.follow?e.clientY||PVI.y:parseInt(PVI.BOX.style.top,10)+PVI.BOX.offsetHeight/2)+"px";
if(animDIV){
...
vdfDpshPtdhhd:"from_frame","reset":true
}
,"*");
if(PVI.state)win.removeEventListener("mousemove",
PVI.m_move,true);
PVI.node=null;
clearTimeout(PVI.timers.delayed_loader);
...
PVI.fullZm=false;
PVI.hideTime=null;
if(PVI.anim.maxDelay)PVI.DIV.style[platform["transition"]]=PVI.anim.css;
...
if(PVI.fireHide&&PVI.state<3)if(cfg.hz.deactivate)PVI.m_over({
"relatedTarget":PVI.TRG
}
);
...
win.addEventListener("keyup",PVI.keyup_freeze,
true);
return
}
if(!e.repeat)if(PVI.keyup_freeze_on)PVI.keyup_freeze();
...
this.removeEventListener(ev.type,
oncopy);
...
if(!e.repeat&&PVI.CNT.src)if(platform.xpi||platform.crx&&!platform.edge)Port.send({
cmd:"download",url:PVI.CNT.src,priorityExt:(PVI.CNT.src.match(/#([\da-z]{
3,4
}
)$/)||[])[1],ext:{
img:"jpg",video:"mp4",audio:"mp3"
}
...
;
((PVI.TRG.IMGS_caption||"").match(/\b((?:www\.[\w-]+(\.\S{
2,7
}
){
1,4
}
|https?:\/\/)\S+)/g)||[]).forEach(function(el){
...
if(key.length){
Port.send({
cmd:"open",url:key,nf:e.shiftKey
}
);
...
else if(key==="Esc")if(PVI.CNT===PVI.VID&&(win.fullScreen||doc.fullscreenElement||topWinW===win.screen.width&&topWinH===win.screen.height))pv=false;
else PVI.reset(true);

else if(key===cfg.keys.hz_fullZm||key==="Enter")if(PVI.fullZm)if(e.shiftKey)PVI.fullZm=PVI.fullZm===1?2:1;
else PVI.reset(true);
else{
...
if(PVI.fullZm)PVI.DIV.style[platform["transition"]]="all 0s"
}
,PVI.anim.maxDelay);
...
if(PVI.CNT===PVI.VID){
pv=true;
if(key==="Space")if(e.shiftKey){
if(!PVI.VID.audio)PVI.VID.controls=PVI.VID._controls=!PVI.VID._controls
}
else if(PVI.VID.paused)PVI.VID.play();
else PVI.VID.pause();

else if(key==="Up"||key==="Down")if(e.shiftKey)PVI.VID.playbackRate*=key==="Up"?4/3:.75;
else pv=null;
else if(!e.shiftKey&&(key==="PgUp"||key==="PgDn"))if(PVI.VID.audio)PVI.VID.currentTime+=key==="PgDn"?4:-4;
else{
PVI.VID.pause();
...
switch(key){
case "End":if(e.shiftKey&&(pv=prompt("#",PVI.stack[PVI.TRG.IMGS_album].search||"")||null))PVI.stack[PVI.TRG.IMGS_album].search=pv;
else pv=false;
break;

case "Home":pv=true;
break;
case "Up":case "Down":pv=null;
break;
default:pv=(key==="Space"&&!e.shiftKey||key==="Right"||key==="PgDn"?1:-1)*(e.shiftKey&&key!=="Space"?5:1)
}
if(pv!==null){
PVI.album(pv,true);
pv=true
...
cfg.keys.rotL||key===cfg.keys.rotR){
...
PVI.DIV.style[platform["transform"]]=PVI.DIV.curdeg?"rotate("+PVI.DIV.curdeg+"deg)":"";
if(PVI.fullZm)PVI.m_move();
else PVI.show()
}
else if(key===cfg.keys.hz_caption)if(e.shiftKey){
PVI.createCAP();
switch(PVI.CAP.state){
case 0:key=cfg.hz.capWH||cfg.hz.capText?1:2;
break;
case 2:key=0;
break;
default:key=cfg.hz.capWH&&cfg.hz.capText?
0:2
}
PVI.CAP.state=key;
PVI.CAP.style.display="none";
...
if(PVI.CNT.src){
Port.send({
"cmd":"open","url":PVI.CNT.src.replace(rgxHash,""),"nf":e.shiftKey
}
);
if(!e.shiftKey&&!PVI.fullZm)PVI.reset()
}
...
cfg.keys.prefs){
Port.send({
"cmd":"open","url":"options.html#settings"
}
);
...
else if(e.target===PVI.VID)if((e.offsetY||e.layerY||0)<Math.min(PVI.CNT.clientHeight-40,2*PVI.CNT.clientHeight/3))PVI.reset(true);
else{
if((e.offsetY||e.layerY||0)<PVI.CNT.clientHeight-
40&&(e.offsetY||e.layerY||0)>2*PVI.CNT.clientHeight/3)if(PVI.VID.paused)PVI.VID.play();
else PVI.VID.pause()
}
else PVI.reset(true);
...
if(e){
if(PVI.fullZm)return;
if(!e.target.IMGS_)if(PVI.lastScrollTRG&&PVI.lastScrollTRG!==e.target)PVI.lastScrollTRG=false;
else if(PVI.lastScrollTRG!==false)PVI.lastScrollTRG=e.target
}
if(PVI.keyup_freeze_on)return;
if(PVI.freeze&&PVI.state>=3){
return
}
if(e){
if(PVI.fireHide)PVI.m_over({
...
if(PVI.state>2&&d>=20)if(e.timeStamp-(PVI.lastScrollTime||
0)<d)d=null;
...
if(PVI.state!==4||!PVI.fullZm&&!PVI.freeze)return;
var s=PVI.TRG.IMGS_SVG?PVI.stack[PVI.IMG.src].slice():[PVI.CNT.naturalWidth,PVI.CNT.naturalHeight];
var k=cfg.keys;
var rot=PVI.DIV.curdeg%180;
viewportDimensions();
if(rot)s.reverse();
if(x===k.mFit)if(winW/winH<s[0]/s[1])x=winW>s[0]?0:k.mFitW;
else x=winH>s[1]?0:k.mFitH;
switch(x){
case k.mFitW:winW-=PVI.DBOX["wpb"];
s[1]*=winW/s[0];
s[0]=winW;
if(PVI.fullZm>
1)PVI.y=0;
break;
case k.mFitH:winH-=PVI.DBOX["hpb"];
s[0]*=winH/s[1];
s[1]=winH;
if(PVI.fullZm>1)PVI.y=0;
break;
case "+":case "-":k=[parseInt(PVI.DIV.style.width,10),0];
...
else if(PVI.DIV.curdeg%360)if(!(PVI.DIV.curdeg%180)){
...
if(PVI.fireHide){
if(e.target&&(e.target.IMGS_||(e.relatedTarget||e).IMGS_&&
e.target===PVI.TRG)){
...
if(PVI.TRG){
if(PVI.DIV)if(PVI.timers.no_anim_in_album){
PVI.timers.no_anim_in_album=null;
PVI.DIV.style[platform["transition"]]=PVI.anim.css
}
PVI.TRG=null
...
trg=e.target;
cache=trg.IMGS_c;
if(!cache)if(trg.IMGS_c_resolved)src=trg.IMGS_c_resolved;
else PVI.TRG=trg;
...
if(!isFrozen&&(!cfg.hz.waitHide||cfg.hz.delay<
15)&&(PVI.fireHide&&PVI.state>2||(PVI.state===2||PVI.hideTime&&Date.now()-PVI.hideTime<200))){
...
PVI.fireHide=true;
if(cfg.hz.markOnHover&&(isFrozen||cfg.hz.delay>=25))if(cfg.hz.markOnHover==="cr"){
...
,load:function(src){
if((cfg.hz.waitHide||!cfg.hz.deactivate)&&PVI.anim.maxDelay&&!PVI.iFrame)win.addEventListener("mousemove",
PVI.m_move,true);
if(!PVI.TRG)return;
if(src===void 0)src=cfg.hz.delayOnIdle&&PVI.TRG.IMGS_c_resolved||PVI.SRC;
...
else if(PVI.lastTRGStyle.cursor!==
null){
if(PVI.DIV)PVI.DIV.style.cursor="";
PVI.TRG.style.cursor=PVI.lastTRGStyle.cursor;
PVI.lastTRGStyle.cursor=null
...
else if(e.target.IMGS_&&PVI.TBOX&&(PVI.TBOX.Left>e.pageX||PVI.TBOX.Right<
e.pageX||PVI.TBOX.Top>e.pageY||PVI.TBOX.Bottom<e.pageY)||!e.target.IMGS_&&PVI.TRG!==e.target)PVI.m_over({
"relatedTarget":PVI.TRG,"clientX":e.clientX,"clientY":e.clientY
}
);
...
for(var i=0,lastIdx=srcs.length-1;
i<=lastIdx;
++i){
var url=srcs[i];
var isHDUrl=url[0]==="#";
...
doc.body.removeEventListener("DOMNodeInserted",
PVI.preload,true)
}
return
}
...
if(node.nodeName.toUpperCase()==="A"&&node.childElementCount||node.IMGS_c_resolved||node.IMGS_c||typeof node.IMGS_caption==="string"||node.IMGS_thumb)continue;
...
for(i=0;
i<src.length;
++i)if(cfg.hz.hiRes&&src[i][0]==="#")img.IMGS_src_arr.push(src[i].slice(1));
...
if(d.hide){
PVI.hide({
target:PVI.TRG,clientX:PVI.DIV.offsetWidth/2+cfg.hz.margin,clientY:PVI.DIV.offsetHeight/2+cfg.hz.margin
}
);
...
if(cfg.tls.sieveCacheRes&&
!d.params.rule.skip_resolve&&d.m)Port.send({
"cmd":"resolve_cache","url":d.params.url,"cache":JSON.stringify(d.m),"rule_id":d.params.rule.id
}
)
}
if(d.m&&!Array.isArray(d.m)&&typeof d.m==="object")if(d.m[""]){
...
if(Array.isArray(d.m))if(d.m.length){
...
if(cfg.hz.capText&&d.m[0])if(d.m[1])PVI.prepareCaption(trg,d.m[1]);
...
if(d.m){
if(!d.noloop&&!trg.IMGS_album&&
typeof d.m==="string"&&(d.loop||rule.loop&&rule.loop&(d.params.rule.loop_param==="img"?2:1))){
d.m=PVI.find({
href:d.m,IMGS_TRG:trg
}
);
...
if(trg===PVI.TRG)if(trg.IMGS_album)PVI.album(d.idx||"1");
else PVI.set(d.m);
else{
...
doc[e]("visibilitychange",
PVI.onVisibilityChange,true);
...
PVI.mutObserverConf=
{
attributes:true,attributeOldValue:true,attributeFilter:["href","src","style","alt","title"]
}
...
script.textContent="dispatchEvent(new CustomEvent('"+evName+"', {
"+"bubbles: false, detail: window['"+varName+"']
}
"+"))";
